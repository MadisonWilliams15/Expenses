...
class ExpensesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return expenses.count //how many expense items in the array
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "expenseCell", for: indexPath)
        
        if let cell = cell as? ExpenseTableViewCell { //cast as expensetableviewcell
            
            let expense = expenses[indexPath.row] //which row are we in
            
            cell.titleLabel.text = expense.title
            cell.amountLabel.text = String(expense.amount) //convert to string for label
            cell.dateLabel.text = displayDateFormatter.string(from: expense.date) //convert to string for label 
        }
